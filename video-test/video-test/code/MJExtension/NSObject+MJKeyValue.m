...
// MARK: aaa
- (instancetype)thb_setKeyValues:(id)keyValues
{
    return [self thb_setKeyValues:keyValues context:nil];
}


- (instancetype)thb_setKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    // 获得JSON对象
    keyValues = [keyValues mj_JSONObject];
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            if (!type.isFromFoundation && propertyClass) { // 模型属性
                value = [propertyClass thb_objectWithKeyValues:value context:context];
            } else if (objectClass) {
                if (objectClass == [NSURL class] && [value isKindOfClass:[NSArray class]]) {
                    // string array -> url array
                    NSMutableArray *urlArray = [NSMutableArray array];
                    ...
                    value = [objectClass thb_objectArrayWithKeyValuesArray:value context:context];
                }
            }
...
+ (instancetype)thb_objectWithKeyValues:(id)keyValues
{
    return [self thb_objectWithKeyValues:keyValues context:nil];
}

+ (instancetype)thb_objectWithKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    // 获得JSON对象
    keyValues = [keyValues mj_JSONObject];
    MJExtensionAssertError([keyValues isKindOfClass:[NSDictionary class]], nil, [self class], @"keyValues参数不是一个字典");
    ...
    return [[[self alloc] init] thb_setKeyValues:keyValues];
}



+ (NSMutableArray *)thb_objectArrayWithKeyValuesArray:(NSArray *)keyValuesArray
{
    return [self thb_objectArrayWithKeyValuesArray:keyValuesArray context:nil];
}

+ (NSMutableArray *)thb_objectArrayWithKeyValuesArray:(id)keyValuesArray context:(NSManagedObjectContext *)context
{
    // 如果是JSON字符串
    keyValuesArray = [keyValuesArray mj_JSONObject];
    ...
    for (NSDictionary *keyValues in keyValuesArray) {
        if ([keyValues isKindOfClass:[NSArray class]]){
            [modelArray addObject:[self thb_objectArrayWithKeyValuesArray:keyValues context:context]];
        } else {
            
            NSString *classStr = keyValues[@"mj_classKey"];
            Class class = NSClassFromString(classStr);
            
            id model = [class mj_objectWithKeyValues:keyValues context:context];
            if (model) [modelArray addObject:model];
        }
    }
...
#pragma mark - 模型 -> 字典
- (NSMutableDictionary *)thb_keyValues
{
    return [self thb_keyValuesWithKeys:nil ignoredKeys:nil];
}

- (NSMutableDictionary *)thb_keyValuesWithKeys:(NSArray *)keys
{
    return [self thb_keyValuesWithKeys:keys ignoredKeys:nil];
}

- (NSMutableDictionary *)thb_keyValuesWithIgnoredKeys:(NSArray *)ignoredKeys
{
    return [self thb_keyValuesWithKeys:nil ignoredKeys:ignoredKeys];
}


- (NSMutableDictionary *)thb_keyValuesWithKeys:(NSArray *)keys ignoredKeys:(NSArray *)ignoredKeys
{
    // 如果自己不是模型类, 那就返回自己
    // 模型类过滤掉 NSNull
    // 唯一一个不返回自己的
    ...
    keyValues[@"mj_classKey"] = NSStringFromClass(clazz);
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            Class propertyClass = type.typeClass;
            if (!type.isFromFoundation && propertyClass) {
                value = [value thb_keyValues];
            } else if ([value isKindOfClass:[NSArray class]]) {
                // 3.处理数组里面有模型的情况
                value = [NSObject thb_keyValuesArrayWithObjectArray:value];
            } else if (propertyClass == [NSURL class]) {
                value = [value absoluteString];
            } else if (type.isSpecialType) {
                value = [self tranfromWithValue:value code:type.code];
            }
...
+ (NSMutableArray *)thb_keyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    return [self thb_keyValuesArrayWithObjectArray:objectArray keys:nil ignoredKeys:nil];
}



+ (NSMutableArray *)thb_keyValuesArrayWithObjectArray:(NSArray *)objectArray keys:(NSArray *)keys ignoredKeys:(NSArray *)ignoredKeys {
    // 0.判断真实性
    MJExtensionAssertError([objectArray isKindOfClass:[NSArray class]], nil, [self class], @"objectArray参数不是一个数组");
    ...
    for (id object in objectArray) {
        if (keys) {
            id convertedObj = [object thb_keyValuesWithKeys:keys];
            if (!convertedObj) { continue; }
            [keyValuesArray addObject:convertedObj];
        } else {
            id convertedObj = [object thb_keyValuesWithIgnoredKeys:ignoredKeys];
            if (!convertedObj) { continue; }
            [keyValuesArray addObject:convertedObj];
        }
    }
...
- (id)tranfromWithValue:(id)value code:(NSString *)code {
    if ([code isEqualToString:MJPropertyTypeCGRect]) {
        CGRect rect = [value CGRectValue];
        return NSStringFromCGRect(rect);
    } else if ([code isEqualToString:MJPropertyTypeCGPoint]) {
        CGPoint point = [value CGPointValue];
        return NSStringFromCGPoint(point);
    } else if ([code isEqualToString:MJPropertyTypeCMTimeRange]) {
        CMTimeRange timeRange = [value CMTimeRangeValue];
        return (__bridge_transfer NSDictionary *)CMTimeRangeCopyAsDictionary(timeRange, kCFAllocatorDefault);
    }
