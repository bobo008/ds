...
@implementation PPPPixelBufferUtil
...
@implementation PPPPixelBufferUtil (vImage)
...
+ (CVPixelBufferRef)correct:(CVPixelBufferRef)pixel orientation:(UIImageOrientation)orientation {
    ...
    CVPixelBufferRef dstPixel = [PPPPixelBufferUtil pixelBufferForWidth:size.width height:size.height format:pixelFormat];
...
+ (CVPixelBufferRef)resizePixelBuffer:(CVPixelBufferRef)sourcePixelBuffer targetSize:(CGSize)targetSize {
    if (!sourcePixelBuffer) return nil;
    
    int height = (int)CVPixelBufferGetHeight(sourcePixelBuffer);
    int width = (int)CVPixelBufferGetWidth(sourcePixelBuffer);
    
    if (targetSize.width > 1.0 * targetSize.height / height * width) {
        targetSize.width = (int)(1.0 * targetSize.height / height * width);
    } else {
        targetSize.height = (int)(1.0 * targetSize.width / width * height);
    }

    CVPixelBufferRef result = NULL;
    @try {
        result = [self _scale:sourcePixelBuffer size:targetSize];
...
+ (CVPixelBufferRef)_scale:(CVPixelBufferRef)pixel size:(CGSize)targetSize {
    ...
    if (pixelFormat == kCVPixelFormatType_OneComponent8) {
        vImage_Error scaleErr = vImageScale_Planar8(&sourceBuffer, &destinationBuffer, NULL, kvImageHighQualityResampling | kvImageNoAllocate);
        NSCAssert(scaleErr == kvImageNoError, @"[vImageAffineWarp_Planar8]: %d", (int)scaleErr);
    } else if (pixelFormat == kCVPixelFormatType_32BGRA) {
        vImage_Error scaleErr = vImageScale_ARGB8888(&sourceBuffer, &destinationBuffer, NULL, kvImageHighQualityResampling | kvImageNoAllocate);
        NSCAssert(scaleErr == kvImageNoError, @"[vImageAffineWarp_ARGB8888]: %d", (int)scaleErr);
    }
    CVPixelBufferUnlockBaseAddress(dstPixel, 0);
...
@implementation PPPPixelBufferUtil (INP)
