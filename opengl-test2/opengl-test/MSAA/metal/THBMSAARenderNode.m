...
- (void)setup {
    ...
    texDescriptor.sampleCount = 2;/// 通过拿步长可以看到其步长 * 4 * 2 了（所有的四个点，放在同一行中了） 所以 这个性能影响挺大的，要求 *2 的计算量，这个东西最后呈现到屏幕上可能也有性能问题，最好重绘到一个sampleCount = 1的纹理上再呈现到屏幕上
...
- (CVPixelBufferRef)getPixelBufferFromBGRAMTLTexture:(id<MTLTexture>)texture {
    ...
    void *ptr = (void *)CFDataGetBytePtr(imageBytes);
//    CVPixelBufferCreateWithBytes(kCFAllocatorDefault,texture.width,texture.height,kCVPixelFormatType_32BGRA,imageBytes,bytesPerRow,NULL,NULL,(__bridge CFDictionaryRef)options,&pxbuffer);
    CVReturn err = CVPixelBufferCreateWithBytes(kCFAllocatorDefault, texture.width, texture.height, kCVPixelFormatType_32BGRA, ptr, bytesPerRow, NULL, imageBytes, (__bridge CFDictionaryRef)options, &pxbuffer);
